...
const getCourses = () =>
  JSON.parse(localStorage.getItem(LS_COURSES) || "[]");
const setCourses = (arr) =>
  localStorage.setItem(LS_COURSES, JSON.stringify(arr || []));
...
async function ensureMappingsLoaded() {
  ...
  if (lessonsJson && typeof lessonsJson === "object") {
    if (lessonsJson.course)
      _courseMap = _mergeMaps(_courseMap, lessonsJson.course);
    else _courseMap = _mergeMaps(_courseMap, lessonsJson);

    if (lessonsJson.room)
      _roomMap = _mergeMaps(_roomMap, lessonsJson.room);
  }
...
async function loadMappings() {
  ...
  COURSE_MAP = {};
  try {
    COURSE_MAP = await loadJson("/lessons_mapped.json");
  } catch {
    try {
      COURSE_MAP = await loadTxtMap("/course_mapping.txt");
    } catch {}
  }
  ...
  ROOM_MAP = {};
  try {
    ROOM_MAP = await loadJson("/rooms_mapped.json");
  } catch {
    try {
      ROOM_MAP = await loadTxtMap("/room_mapping.txt");
    } catch {}
  }
}
...
function buildCourseSelection(allLessons) {
  ...
  subjects.forEach((sub) => {
    ...
    label.className = "chk";
    label.innerHTML = `<input type="checkbox" id="${id}" value="${sub}" ${
      saved.has(sub) ? "checked" : ""
    }> <span>${niceCourse(sub)}</span>`;
...
function buildGrid(lessons) {
  ...
  valid.forEach((l) => {
    ...
    const s = parseHM(l.start),
      e = parseHM(l.end);
    const r0 = rowIndexFor(s),
      r1 = rowIndexFor(e);
